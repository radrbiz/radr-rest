...
var RadrdNetworkError   = errors.RadrdNetworkError;
...
function handleError(error, req, res, next) {
  ...
  /* Handle radr-lib errors */
  if (err_obj.error === 'remoteError') {
    if (err_obj.message === 'Account not found.') {
      err_obj.error = 'Invalid address or secret.';
      err_obj.message = 'Please ensure the address and secret correspond to a valid account that has a positive VRP balance.';

    } else {
      err_obj.error = 'Internal Error';
      err_obj.message = 'radr-lib reported an error. If the problem persists, please try restarting the server. Error: ' + JSON.stringify(err_obj.message);
    }
  }
  ...
  if (err_obj.error === 'telINSUF_FEE_P') {
    err_obj.message = err_obj.message + ' Please ensure that the source_address has sufficient VRP to pay the fee. If it does, please report this error, this service should handle setting the proper fee.';
  }
