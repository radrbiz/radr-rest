  ...
  .then(function(exists) {
    if (!exists) {
      ...
      .createTable('transaction_history', function(table) {
        ...
        table.text('engine_result').nullable();
      });
    }
  })
  .then(function(res) {
...
DI.saveTransaction = function(transaction, callback) {
  ...
  if (result.engine_result) {
    txData.engine_result = result.engine_result;
  }
  ...
  .then(function(res) {
    ...
    var info = txData.hash + ': ' + txData.engine_result;
...
DI.getFailedTransactions = function(options, callback) {
  ...
  assert(radr.UInt160.is_valid(options.account),
...
DI.convertOutgoingTransaction = function(txEntry) {
  ...
  transaction.meta.TransactionResult = txEntry.engine_result;
