...
var parseBalanceChanges   = require('radr-lib-transactionparser').parseBalanceChanges;
var parseOrderBookChanges = require('radr-lib-transactionparser').parseOrderBookChanges;
...
TxToRestConverter.prototype.parsePaymentFromTx = function(tx, options, callback) {
  ...
  var payment = {
    ...
    source_amount: (
      ...
      tx.SendMax ?
        ...
        ( (tx.SendMax.currency === 'VBC' || tx.SendMax.currency === 'VRP') ?
           utils.dropsToXrp(tx.SendMax) : tx.SendMax
        )

        :

        ((Amount.currency === 'VBC' || Amount.currency === 'VRP') ?
          utils.dropsToXrp(tx.Amount) : Amount
        )
    ),
    source_slippage: '0',
    ...
    destination_amount: (
      (Amount.currency === 'VBC' || Amount.currency === 'VRP') ?
        utils.dropsToXrp(Amount) : Amount
    ),
    // Advanced options
    ...
    timestamp: (tx.date ? new Date(radr.utils.toTimestamp(tx.date)).toISOString() : ''),
  ...
  if (isPartialPayment && tx.meta && tx.meta.DeliveredAmount) {
    payment.destination_amount_submitted = ((tx.Amount.currency === 'VRP' || tx.Amount.currency === 'VBC') ?
      utils.dropsToXrp(tx.Amount) : tx.Amount
    );
    payment.source_amount_submitted = (tx.SendMax ?
      ((tx.SendMax.currency === 'VRP' || tx.SendMax.currency === 'VBC') ?
        utils.dropsToXrp(tx.SendMax) : tx.SendMax
      )
      :
      ((tx.Amount.currency === 'VRP' || tx.Amount.currency === 'VBC') ?
      utils.dropsToXrp(tx.Amount)
      :
      tx.Amount));
  }
...
TxToRestConverter.prototype.parseOrderFromTx = function(tx, options) {
  return new Promise(function(resolve, reject) {
    ...
    var timestamp = tx.date ? new Date(radr.utils.toTimestamp(tx.date)).toISOString() : '';
    var order_changes = tx.meta ? parseOrderBookChanges(tx.meta)[options.account] || [] : [];
 ...
 *  @param {RadrAddress} options.source_account Since this is not returned by rippled in the pathfind results it can either be added to the results or included in the options here
 *
...
TxToRestConverter.prototype.parsePaymentsFromPathFind = function(pathfindResults, callback) {
  ...
  pathfindResults.alternatives.forEach(function(alternative) {
    var payment = {
      ...
      source_tag: '',
      source_amount: ((alternative.source_amount.currency === 'VRP' || alternative.source_amount.currency === 'VBC') ?
      utils.dropsToXrp(alternative.source_amount)
      :
      {
      ...
      destination_tag: '',
      destination_amount: ((pathfindResults.destination_amount.currency === 'VRP' || pathfindResults.destination_amount.currency === 'VBC') ?
      utils.dropsToXrp(pathfindResults.destination_amount)
      :
      {
...
const OfferCreateFlags = {
  Passive:           { name: 'passive', value: radr.Transaction.flags.OfferCreate.Passive },
  ImmediateOrCancel: { name: 'immediate_or_cancel', value: radr.Transaction.flags.OfferCreate.ImmediateOrCancel },
  FillOrKill:        { name: 'fill_or_kill', value: radr.Transaction.flags.OfferCreate.FillOrKill },
  Sell:              { name: 'sell', value: radr.Transaction.flags.OfferCreate.Sell }
};
...
TxToRestConverter.prototype.parseSubmitOrderFromTx = function(message, meta, callback) {
  ...
  _.extend(result.order, {
    ...
    fee: utils.dropsToXrp(message.tx_json.Fee),
    type: (message.tx_json.Flags & radr.Transaction.flags.OfferCreate.Sell) > 0 ? 'sell' : 'buy',
    sequence: message.tx_json.Sequence
  });
...
const TrustSetResponseFlags = {
  NoRipple:      { name: 'prevent_rippling', value: radr.Transaction.flags.TrustSet.NoRipple },
  SetFreeze:     { name: 'account_trustline_frozen', value: radr.Transaction.flags.TrustSet.SetFreeze },
  SetAuth:       { name: 'authorized', value: radr.Transaction.flags.TrustSet.SetAuth }
};
...
const AccountSetResponseFlags = {
  RequireDestTag: { name:   'require_destination_tag', value: radr.Transaction.flags.AccountSet.RequireDestTag },
  RequireAuth:    { name:   'require_authorization', value: radr.Transaction.flags.AccountSet.RequireAuth },
  DisallowXRP:    { name:   'disallow_xrp', value: radr.Transaction.flags.AccountSet.DisallowXRP }
};
