...
RestToTxConverter.prototype.convert = function(payment, callback) {
  try {
    ...
    if (payment.source_amount && (payment.source_amount.currency !== 'VRP' || payment.source_amount.currency !== 'VBC') && payment.source_amount.issuer === '') {
      payment.source_amount.issuer = payment.source_account;
    }
    if (payment.destination_amount && (payment.source_amount.currency !== 'VRP' || payment.source_amount.currency !== 'VBC') && payment.destination_amount.issuer === '') {
    ...
    /* Construct payment */
    var transaction = new radr.Transaction();
    var transactionData = {
      from: payment.source_account,
      to: payment.destination_account
    };
    if (payment.destination_amount.currency === 'VRP' || payment.destination_amount.currency === 'VBC') {
      transactionData.amount = utils.xrpToDrops(payment.destination_amount);
    } else {
    ...
    if (payment.source_amount) {
      ...
      if (!(payment.source_amount.currency === payment.destination_amount.currency && payment.source_amount.issuer === payment.destination_amount.issuer)) {
        ...
        if (payment.source_amount.currency === 'VRP' || payment.source_amount.currency === 'VBC') {
          transaction.sendMax(utils.xrpToDrops({value: max_value, currency: payment.source_amount.currency, issuer: payment.source_amount.issuer}).value);
        } else {
          transaction.sendMax(max_value);
        }
      }
    }
